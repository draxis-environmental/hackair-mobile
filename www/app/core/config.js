(function() {
  ...
  configure.$inject = ['$logProvider', 'routerHelperProvider', '$httpProvider'];
  /* @ngInject */
  function configure($logProvider, routerHelperProvider, $httpProvider, $rootScope) {
  ...
  function LanguageInterceptor($q, $injector, $localStorage, API_URL, $rootScope){
    return {
      request: function requestCallback(config){
        var lang;

        if ($rootScope.global_language == undefined){
          lang = 'en';
        } else {
          lang = $rootScope.global_language;
        }
